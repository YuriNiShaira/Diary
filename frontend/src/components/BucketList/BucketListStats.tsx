  ...
  return (
    <>
      ...
      >
        {[
          {
            icon: <Target className={`w-6 h-6 mx-auto mb-2 ${theme === 'dark' ? 'text-purple-300' : 'text-rose-500'}`} />,
            value: stats.total,
            label: 'Total Dreams',
            bg: theme === 'dark' ? 'bg-slate-950 border-slate-800 text-slate-100' : 'bg-white border-slate-200 text-slate-900',
          },
          {
          ...
          >
            <div className="absolute inset-x-6 top-5 h-1.5 rounded-full bg-slate-200/80" />
            <div className="relative pt-6">
              {card.icon}
              <p className={`text-3xl font-semibold ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{card.value}</p>
              <p className={`text-sm ${theme === 'dark' ? 'text-slate-300' : 'text-slate-500'}`}>{card.label}</p>
            </div>
          </div>
        ))}
      </motion.div>
      ...
      >
        <div className={`relative rounded-4xl border p-6 shadow-sm notebook-card ${
          ...
          <div className="relative pt-5">
            <p className={`text-sm mb-3 ${theme === 'dark' ? 'text-slate-300' : 'text-slate-500'}`}>Overall Progress</p>
            <div className={`h-4 rounded-full overflow-hidden ${theme === 'dark' ? 'bg-slate-800' : 'bg-rose-100'}`}>
              <motion.div
                initial={{ width: 0 }}
                animate={{ width: `${stats.completion_rate}%` }}
                transition={{ duration: 1, ease: 'easeOut' }}
                className="h-full bg-[#ff6b8b]"
              />
            </div>
            <p className={`text-xs mt-2 ${theme === 'dark' ? 'text-slate-300' : 'text-slate-500'}`}>
