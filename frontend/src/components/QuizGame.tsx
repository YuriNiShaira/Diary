...
const QuizGame: React.FC<QuizGameProps> = ({ yearId, yearNumber, onBack, currentScore, onReset }) => {
  ...
  const { data: questions, refetch: refetchQuestions } = useQuery<QuizQuestion[]>({
  ...
  const handleResetScore = () => {
    if (confirm('Reset all quiz scores? This will also reset all questions to unused!')) {
      onReset();
    }
  };
  ...
  const categories = [
    ...
    { value: 'about_shaira', label: 'About Shaira ' },
  ...
  const difficulties = [
    { value: 'easy', label: 'Easy', points: 1, color: 'from-green-400 to-emerald-400', icon: Star },
    { value: 'medium', label: 'Medium', points: 3, color: 'from-yellow-400 to-amber-400', icon: Zap },
    { value: 'hard', label: 'Hard', points: 5, color: 'from-red-400 to-pink-400', icon: Award },
  ];
    ...
    >
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-4">
          <div className="text-center">
            <p className="text-sm text-gray-500">Yuri</p>
      ...
      {currentQuestion ? (
          ...
          <div className="p-6 bg-gradient-to-r from-pink-50 to-rose-50 rounded-2xl">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-sm text-gray-500">
                {currentQuestion.category_display}
              </span>
            </div>
            ...
            {showHint && currentQuestion.hint && (
              <motion.div
                ...
                className="p-3 bg-yellow-50 rounded-xl mb-4"
              >
        ...
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 mb-6">
            Pick a random question or create your own!
          </p>
        </div>
      )}
      ...
      <div className="flex gap-2 mb-4">
        ...
        >
          ...
          {difficulties.map(d => (
            <option key={d.value} value={d.value}>{d.label} ({d.points} pts)</option>
          ))}
        </select>
        <select
      ...
      <div className="space-y-2 max-h-64 overflow-y-auto">
        <h4 className="font-medium text-gray-700 mb-2">
          Question Bank ({filteredQuestions?.length || 0})
        </h4>
          ...
          <div key={question.id} className="p-3 bg-white/60 rounded-lg flex items-center justify-between">
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 mt-1">
                <span className={`text-xs px-2 py-0.5 rounded-full text-white bg-gradient-to-r ${difficulties.find(d => d.value === question.difficulty)?.color}`}>
                  {question.points} pts
                </span>
            ...
            <div className="flex gap-1">
              <button
                ...
                className="p-1 text-gray-400 hover:text-love-red"
              >
          ...
          >
            ...
            >
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => setIsQuestionModalOpen(false)} className="text-gray-400 hover:text-gray-600">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <form onSubmit={(e) => {
                ...
                <div>
                  ...
                  <select
                    value={questionForm.created_by}
                    onChange={(e) => setQuestionForm({ ...questionForm, created_by: e.target.value })}
                    className="w-full px-3 py-2 border border-pink-200 rounded-lg"
                  >
