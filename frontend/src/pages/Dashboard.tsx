...
const Dashboard: React.FC = () => {
  ...
  // OPTIMIZATION 1: Optimized query with caching
  ...
  // OPTIMIZATION 2: Memoize years array
  const years = useMemo(() => 
    Array.isArray(yearsData) ? yearsData : []
  , [yearsData]);

  // OPTIMIZATION 3: Batch stats fetching
  ...
  // OPTIMIZATION 4: Combined useEffect with cleanup
  useEffect(() => {
    fetchStats();
    if (user && !user.has_partner) {
      fetchInviteCode();
    }
  }, [fetchStats, fetchInviteCode, user]);

  // OPTIMIZATION 5: Memoized copy function
  ...
  // OPTIMIZATION 6: Memoized handlers
  ...
  const handleYearClick = useCallback((yearId: number) => navigate(`/year/${yearId}`), [navigate]);

  // OPTIMIZATION 7: Memoize stats cards data to prevent recreation
  ...
  // OPTIMIZATION 8: Check if empty state should show
  ...
  return (
    <div className="min-h-screen relative overflow-hidden transition-colors duration-300">
      <style dangerouslySetInnerHTML={{__html: `
        ...
        /* OPTIMIZATION 9: Reduce paint operations */
        .will-change-transform {
          will-change: transform;
        }
        
        /* OPTIMIZATION 10: GPU acceleration for animations */
        .gpu-accelerated {
      ...
      <div className="max-w-7xl mx-auto relative z-10 px-4 sm:px-6 py-4 sm:py-6">
        {/* Welcome Header - Reduced animation complexity */}
        <motion.div 
        ...
        >
          ...
          <div className="text-center">
            ...
            <p className={`text-base sm:text-lg md:text-xl font-serif italic tracking-wide mt-3 sm:mt-4 ${theme === 'dark' ? 'text-rose-200/80' : 'text-rose-800/70'}`}>
        ...
        {/* Time Counter - Reduced animation */}
        <motion.div 
        ...
        {/* Stats Grid - Optimized with useMemo */}
        <motion.div 
          initial={{ opacity: 0 }} 
          animate={{ opacity: 1 }} 
        ...
        >
          <h2 className={`text-3xl sm:text-4xl font-serif text-center mb-6 sm:mb-8 tracking-wide ${theme === 'dark' ? 'text-rose-100' : 'text-rose-950'}`}>
            <span className="text-gradient-soft">Our Journey Through The Years</span>
            <motion.div 
              initial={{ width: 0 }} 
              animate={{ width: '100px' }} 
              transition={{ delay: 0.35, duration: 0.8 }} 
              className="h-px bg-linear-to-r from-transparent via-rose-300 to-transparent mx-auto mt-3 sm:mt-4" 
            />
          </h2>
          ...
          {isLoading ? (
            ...
            <>
              ...
              >
                ...
                >
                  <div className={`absolute inset-1.5 border border-dashed rounded-full opacity-50 pointer-events-none ${theme === 'dark' ? 'border-rose-900' : 'border-rose-200'}`} />
                  
                  <span className={`relative z-10 flex items-center gap-2 sm:gap-3 font-serif uppercase tracking-[0.15em] sm:tracking-[0.2em] text-[10px] sm:text-xs font-bold ${theme === 'dark' ? 'text-rose-200' : 'text-rose-800'}`}>
                    <Sparkles className={`w-3 h-3 sm:w-4 sm:h-4 transform group-hover:scale-110 transition-transform ${theme === 'dark' ? 'text-rose-400' : 'text-rose-400'}`} />
                    Begin a New Chapter
                    <Sparkles className={`w-3 h-3 sm:w-4 sm:h-4 transform group-hover:scale-110 transition-transform ${theme === 'dark' ? 'text-rose-400' : 'text-rose-400'}`} />
                  </span>
                </motion.button>
          ...
          {showEmptyState && (
            ...
            >
              ...
              <h3 className={`text-xl sm:text-2xl font-serif mb-2 sm:mb-3 ${theme === 'dark' ? 'text-rose-200' : 'text-rose-800'}`}>
                Start Your Love Story
              </h3>
              <p className={`mb-6 sm:mb-8 font-serif italic text-sm sm:text-base ${theme === 'dark' ? 'text-rose-300/70' : 'text-rose-700/60'}`}>
        ...
        <motion.div 
          ...
          className="mt-16 sm:mt-20 text-center pb-6 sm:pb-8"
        >
          <p className={`font-serif italic text-xs sm:text-sm ${theme === 'dark' ? 'text-rose-300/60' : 'text-rose-700/50'}`}>
      ...
      {/* Invite Code Modal - Optimized with lazy loading */}
            ...
            >
              ...
              <div className="text-center mt-2 mb-4 sm:mb-6">
                <div className="flex justify-center mb-2 sm:mb-3">
                  <div className={`p-2 sm:p-3 rounded-full border-2 border-dashed ${theme === 'dark' ? 'border-rose-900/50 bg-rose-900/20' : 'border-rose-200 bg-rose-50'}`}>
                    <Users className={`w-5 h-5 sm:w-6 sm:h-6 ${theme === 'dark' ? 'text-rose-300' : 'text-rose-500'}`} />
                  </div>
                </div>
                <h3 className={`text-3xl sm:text-4xl font-serif font-bold italic mb-1 sm:mb-2 ${theme === 'dark' ? 'text-rose-200' : 'text-rose-600'}`}>
                  Invitation
                </h3>
                <p className={`font-serif text-xs sm:text-sm px-2 ${theme === 'dark' ? 'text-stone-400' : 'text-stone-500'}`}>
              ...
              <div 
                ...
                title="Click to copy"
              >
                <span className={`font-mono text-2xl sm:text-3xl tracking-[0.2em] sm:tracking-[0.25em] font-bold ${theme === 'dark' ? 'text-stone-200' : 'text-stone-700'}`}>
                  {inviteCode || '...'}
                </span>
